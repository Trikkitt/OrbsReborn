        ...
        if (uart.any()>0):
            ...
            if OrbFunctions.crc16(cmd)==0:
                if cmd[0]<101: # Message for server
                    e.send(gamehost,cmd)
                if cmd[0]==0x92: # Message with config
                    if cmd[1]==6: # New game host
                        gamehost=cmd[2,3,4,5,6,7]
                        myconfig["gamehost"]=gamehost
                        OrbFunctions.save_config(myconfig)
                    if cmd[1]==7: # discover game host
                        gamehost=OrbFunctions.DiscoverHost(e,sta,uart)
                        myconfig["gamehost"]=gamehost
                        OrbFunctions.save_config(myconfig)
                if cmd[0]==0xCD:
